...
frames = sorted(glob("./frames/test/*.jpg"))

out_path = "./test_detectresults_out"
...
for i,image in tqdm(enumerate(frames),total=len(frames)):
    ...
    for a,value in enumerate(list(outputs)):
        ...
        file_pathpose = out_path + '/person' + str(track_id).zfill(8) + '/HRnet_pose'
        file_pathimg = out_path + '/person' + str(track_id).zfill(8) + '/img'
