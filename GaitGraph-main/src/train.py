...
def main(opt):
    ...
    transform = transforms.Compose(
        [
            MirrorPoses(opt.mirror_probability),   #镜像
            FlipSequence(opt.flip_probability),    #倒走
            RandomSelectSequence(opt.sequence_length),  #随机选取60帧数
            ShuffleSequence(opt.shuffle),           #打乱
            PointNoise(std=opt.point_noise_std),    #加噪声
